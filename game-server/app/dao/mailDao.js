...
mailDao.systemSendMail = function(toBox , mail, cb ) {
    ...
    redis.command(function(client) {
        ...
        }).lpush(toBox,receiveMail,function(err, reply) {
                if( !!err ) {
...
mailDao.DelInboxMail = function(Key, cb) {
    ...
    redis.command(function(client) {
        ...
        }).rpop(Key+"_"+MailKeyType.READ,function(err,reply) {
            if(!!err){
...
mailDao.collectItem=function(Key,mails,itemIndex,player,cb){
    ...
    mailDao.delMail(mails, Key, function(err, reply) {
        ...
        item.hasCollect = 1;
        var i = player.packageEntity.addItemWithNoType(item);
        logger.error(i);
        var allCollect = 1;
